...
try {
  ...
  const envPath = path.join(PROJECT_ROOT, '.local.env');
  ...
  if (process.env.OPENAI_API_KEY) {
    ...
    console.log("❌ OpenAI API key not found after loading .env.local");
  }
} catch (error) {
...
async function main() {
  try {
    ...
    if (!process.env.OPENAI_API_KEY && !args.openaiKey) {
      console.error("Error: OpenAI API key not found. Set OPENAI_API_KEY environment variable or use --openai-key option.");
      process.exit(1);
    }
    ...
    console.log("Processing with options:", {
      ...
      huggingFaceKey: options.huggingFaceKey ? "***" : "Not provided (will use keyword fallback)"
    });
    ...
    if (args.file) {
      console.log(`Reading transcript from file: ${args.file}`);
      const transcript = fs.readFileSync(args.file, 'utf-8');
    ...
    // Default: Process from default transcript file
      ...
      if (fs.existsSync(DEFAULT_TRANSCRIPT_FILE)) {
        console.log(`Reading transcript from default file: ${DEFAULT_TRANSCRIPT_FILE}`);
        const transcript = fs.readFileSync(DEFAULT_TRANSCRIPT_FILE, 'utf-8');
        result = await processTranscript(transcript, options);
      } else {
        console.error(`Error: Default transcript file not found at ${DEFAULT_TRANSCRIPT_FILE}`);
        console.error("Please create this file or specify --file, --text, or --summary argument");
        showHelp();
        process.exit(1);
      }
    }
    ...
    if (result.success) {
      ...
      if (result.classification) {
        ...
        console.log(`Scores: ${result.classification.labels[0]}: ${result.classification.scores[0]}%, ` +
        ...
        fs.writeFileSync(outputFile, JSON.stringify(result.classification, null, 2), 'utf-8');
